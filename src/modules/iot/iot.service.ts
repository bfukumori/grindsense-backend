...
export class IotService {
  ...
  async claimDevice(userId: string, macAddress: string) {
    ...
    if (!device) throw new AppError(404, 'Dispositivo não encontrado ou não fabricado.');
  ...
  async processBatch(events: TelemetryEvent[]) {
    ...
    return await tracer.startActiveSpan(`IoT: Process Batch (${events.length})`, async (span) => {
      try {
        ...
        if (missingDeviceIds.length > 0) {
          ...
          for (const device of dbDevices) {
            if (device.userId) {
              ...
              cachePipeline.set(`device:user:${device.id}`, device.userId, 'EX', 86400);
            }
          }
          ...
          return [
            {
              userId,
              deviceId: e.deviceId,
              measuredAt: Temporal.Instant.fromEpochMilliseconds(e.payload.timestamp),
              bpm: e.payload.bpm,
              spo2: e.payload.spo2,
            },
          ];
